...
app.post('/api/create-token', async (req, res) => {
  const { name, symbol, supply, decimals, creator, network } = req.body;
  ...
  let rpcUrl;
  if (network === 'mainnet-beta') {
    rpcUrl = process.env.HELIUS_MAINNET_URL || 'https://api.mainnet-beta.solana.com';
  } else {
    rpcUrl = process.env.HELIUS_DEVNET_URL || 'https://api.devnet.solana.com';
  }

  const connection = new Connection(rpcUrl, 'confirmed');

  try {
    // Use server wallet or create new one
    const payerKeypair = creatorKeypair;
